...
export default function App() {
  ...
  const completeMission = () => {
    ...
    if (nx >= 100) {
      ...
      setScore((v) => v + 11)
      setSavings((v) => v + 150)
      setTip('Amazing streak. Your consistency predicts a +34 credit score improvement in 90 days.')
    } else {
      setXp(nx)
      setSavings((v) => v + 25)
    }
  ...
  const analyzeMood = () => {
    if (journal.toLowerCase().includes('stress'))
      setTip('Money stress detected. AI recommends a 7-day micro-budget plan and automatic savings of $5/day.')
    else if (journal.toLowerCase().includes('travel'))
      setTip('Travel goal detected. Save $18/week to reach a $900 trip fund in 12 months.')
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 md:p-10">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Card>
            ...
            <div className="mt-4 bg-slate-800 rounded-full h-4 overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: progress }} className="h-full bg-cyan-400" />
            </div>
            <div className="text-sm mt-2">{xp}% to next level</div>
          </Card>

          <Card>
            <div className="text-xl font-semibold">AI Future Twin</div>
          ...
          <Card>
            <div className="text-xl font-semibold">Daily Missions</div>
            <button onClick={completeMission} className="w-full mt-4 rounded-2xl bg-cyan-500 p-3 font-semibold">
              Save $15 Today (+XP)
            </button>
            <button onClick={completeMission} className="w-full mt-3 rounded-2xl bg-indigo-500 p-3 font-semibold">
              No Takeout (+XP)
            </button>
            <button onClick={completeMission} className="w-full mt-3 rounded-2xl bg-emerald-500 p-3 font-semibold">
              Pay Bill On Time (+XP)
            </button>
          </Card>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            <div className="flex items-center gap-2 text-xl font-semibold">
              <Brain /> OpenAI Smart Coach
            </div>
            <p className="mt-4 text-slate-300">{tip}</p>
            <button onClick={generateCoach} className="mt-4 rounded-2xl bg-purple-600 px-4 py-2">
          ...
          <Card>
            ...
            <textarea
              ...
              placeholder="Type: I feel stressed about rent / I want to travel..."
              className="mt-4 w-full h-28 rounded-2xl bg-slate-800 p-3"
            />
            <button onClick={analyzeMood} className="mt-4 rounded-2xl bg-pink-600 px-4 py-2">
              Analyze with AI
            </button>
          </Card>
        </div>
        ...
        <Card>
          <div className="flex items-center gap-2 text-xl font-semibold">
            <TrendingUp /> Why Judges Love This
          </div>
...
function Card({ children }) {
  return <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl">{children}</div>
}
...
function Badge({ text }) {
  return <div className="rounded-2xl bg-slate-800 p-3">{text}</div>
}
